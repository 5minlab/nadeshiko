...
import { RecordType, Table } from '../../src/sheets'
import { TableCache } from '../../src/cache'
...
describe('saveTable - loadTable', () => {
	test('number id', async () => {
		...
		expect(await cache.loadTable(name)).toEqual(table)
	})
	...
	test('table not found', async () => {
		const name = faker.random.alphaNumeric(8)
		expect(await cache.loadTable(name)).toEqual(new Table(name, []))
	})
})
